...
class PaintApplication(tk.Frame):
    ...
    def __init__(self, root):
        ...
        self.tool_bar_functions = (
            "draw_irregular_line", "draw_irregular_line"
        )
    ...
    def open_image(self):
        self.file_name = filedialog.askopenfilename(
            master=self.root, filetypes=[("All Files", "*.*")], title="Open...")
        ...
        self.canvas.config(width=self.canvas.img_width,
                           height=self.canvas.img_height)
        self.canvas.create_image(
            self.canvas.img_width/2.0, self.canvas.img_height/2.0, image=self.canvas.tk_img)
    ...
    def import_mask(self):
        file_name_mask = filedialog.askopenfilename(
            master=self.root, filetypes=[("All Files", "*.*")], title="Import mask...")
        mask_img = Image.open(file_name_mask)
        if (mask_img.size != self.canvas.img.size):
            messagebox.showerror(
                "Import mask", "Mask image size does not match the original image size! Mask image not imported.")
            return
    ...
    def on_decensor_menu_clicked(self, event=None):
        ...
        decensorer.decensor_image(
            combined_img.convert('RGB'), self.file_name + ".png")
        messagebox.showinfo(
    ...
    def draw_irregular_line(self):
        ...
        # self.current_item = self.canvas.create_line(
        #   (self.start_x, self.start_y, self.end_x, self.end_y), fill=self.fill, width=int(self.brush_width))
    ...
    def create_menu(self):
        ...
        self.filemenu.add_command(
            label="Open", command=self.on_open_image_menu_clicked)
        self.filemenu.add_command(
        ...
        self.editmenu.add_command(
            label="Undo", command=self.on_undo_menu_clicked)
        self.menubar.add_cascade(label="Edit", menu=self.editmenu)

        self.viewmenu.add_command(
            label="Zoom in", command=self.on_canvas_zoom_in_menu_clicked)
        self.viewmenu.add_command(
            label="Zoom out", command=self.on_canvas_zoom_out_menu_clicked)
    ...
    def create_drawing_canvas(self):
        ...
        self.canvas.img = Image.open(
            './icons/canvas_top_test.png').convert('RGBA')
